...
BOOL SetPrivilege(HANDLE token, LPCWSTR privilege, BOOL enable)
{
  ...
  if (!AdjustTokenPrivileges(token, FALSE, &tokenPriv, sizeof(TOKEN_PRIVILEGES),
...
BOOL SetOwner(LPCTSTR filename, LPCTSTR newOwner)
{
  ...
  LookupAccountName(nullptr, newOwner, nullptr, &sidSize, domainUnused, &domainBufSize,
                    &sidUse);
  sid = (PSID)malloc(sidSize);
  // determine sid for account name
  if (!LookupAccountName(nullptr, newOwner, sid, &sidSize, domainUnused, &domainBufSize,
                         &sidUse)) {
    ...
    secRes =
        SetNamedSecurityInfo(fileNameBuf, SE_FILE_OBJECT,

                              ...
                              const std::wstring& accountName)
{
  if (!exists(directory)) {
    if (!create_directory(directory)) {
      error(L"Failed to create \"{}\", do you have the necessary access rights to the "
  ...
  if (!SetOwner(directory.c_str(), accountName.c_str())) {
    error(L"failed to set owner of \"{}\" to \"{}\"", directory.native(),
          accountName.c_str());
    return false;
  }
...
static bool init(const std::filesystem::path& mopath, const std::wstring& accountName)
{
  if (!SetOwner(mopath.c_str(), accountName.c_str())) {
    error(L"failed to set owner of \"{}\" to \"{}\"", mopath.native(),
          accountName.c_str());
    return false;
  }
...
static bool backdateBSAs(const std::filesystem::path& dataPath)
{
  for (auto& entry : std::filesystem::directory_iterator{dataPath}) {
    ...
    if (!::SetFileTime(file, nullptr, nullptr, &newWriteTime)) {
      error(L"failed to change date for {}: errorcode {}", path.c_str(),
            ::GetLastError());
      return false;
    }
...
static bool adminLaunch(DWORD processID, const std::filesystem::path& executable,
...
#define TIMEOUT_S 30
  ...
  if (processHandle == NULL) {
    error(L"Failed to get process handle for pid {}: errorcode {}", processID,
          ::GetLastError());
    return false;
  }
  ...
  while (t_start + TIMEOUT_S > time(nullptr)) {
    if (!GetExitCodeProcess(processHandle, &exitCode)) {
      error(L"Failed to get process exit code for pid {}: errorcode {}", processID,
            ::GetLastError());
      return false;
    }

    if (exitCode != STILL_ACTIVE) {
      if (!::ShellExecuteW(nullptr, L"runas", executable.c_str(), L"",
                           workingDir.c_str(), SW_SHOWNORMAL)) {
        error(L"Failed to launch {}: errorcode {}", executable.c_str(),
              ::GetLastError());
        return false;
      }
      return true;
